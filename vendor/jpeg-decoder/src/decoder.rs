...
impl<R: Read> Decoder<R> {
    ...
    fn decode_planes(
        ...
        if frame.coding_process == CodingProcess::Lossless {
            let decoded = compute_image_lossless(frame, planes_u16)?;
...
fn compute_image_into(
    ...
    let expected_len = output_size.width as usize
    ...
    if components.len() == 1 {
        ...
        for y in 0..height {
            let source_idx = y * line_stride;
            let destination_idx = y * width;
            output[destination_idx..destination_idx + width]
                .copy_from_slice(&decoded[source_idx..source_idx + width]);
        }
        Ok(())
    } else {
        compute_image_parallel_into(components, &data, output_size, color_transform, output)
    }
}
