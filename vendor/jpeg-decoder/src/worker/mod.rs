...
pub trait Worker {
    ...
    fn append_row_reusable(&mut self, index: usize, row: &mut Vec<i16>) -> Result<()> {
        let replacement = alloc::vec![0; row.len()];
        self.append_row((index, core::mem::replace(row, replacement)))
    }
...
pub fn compute_image_parallel_into(
    components: &[Component],
    data: &[Vec<u8>],
    output_size: Dimensions,
    color_transform: ColorTransform,
    ...
    let upsampler = Upsampler::new(components, output_size.width, output_size.height)?;
    let line_size = output_size.width as usize * components.len();
    let expected_len = line_size * output_size.height as usize;
    ...
    for (row, line) in output.chunks_mut(line_size).enumerate() {
        upsampler.upsample_and_interleave_row(
            data,
            row,
            output_size.width as usize,
